...
import { CalendarDays, Edit3, Eye, Gift, Plus, Save, Tag, Trash2, Upload } from "lucide-react";
...
export default function AdminPromoPage() {
	...
	const selectedPromo = useMemo(() => promos.find((item) => item.id === selectedPromoId) ?? null, [promos, selectedPromoId]);
	...
	async function handleCreatePromo(event: React.FormEvent<HTMLFormElement>) {
		...
		try {
			...
			setSubmitError(error instanceof Error ? error.message : "Gagal menambah promo");
		} finally {
	...
	async function handleUpdatePromo(event: React.FormEvent<HTMLFormElement>) {
		...
		try {
			...
			setActionError(error instanceof Error ? error.message : "Gagal mengubah promo");
		} finally {
			setIsSaving(false);
		}
	}
	...
	async function handleDeletePromo(promoId: number) {
		...
		try {
			...
			setActionError(error instanceof Error ? error.message : "Gagal menghapus promo");
		}
	}
				...
				<section className="flex min-w-0 flex-col bg-[#F0F4FA]">
					<header className="flex flex-col gap-3 border-b border-slate-200 bg-white px-5 py-3 lg:flex-row lg:items-center lg:justify-between">
						<div>
							...
							<div className="text-[9px] text-slate-500">Data diambil dari tabel promo pada database</div>
						</div>
					...
					<div className="grid flex-1 gap-4 overflow-y-auto p-4 xl:grid-cols-[minmax(0,1fr)_380px] lg:p-5">
						<section className="space-y-3">
							<form onSubmit={handleCreatePromo} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm space-y-3">
								<div className="flex items-center justify-between gap-2">
									<div className="text-[12px] font-medium text-slate-900">Tambah promo</div>
									<span className="rounded-full bg-sky-50 px-2 py-1 text-[8px] font-semibold text-sky-600">POST /api/promo</span>
								</div>
								<input value={form.url} onChange={(event) => setForm({ url: event.target.value })} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-[10px] outline-none" placeholder="URL gambar promo" />
							...
							<div className="flex flex-wrap gap-2 text-[9px] font-medium">
								{[`Semua (${promos.length})`, "Aktif", "Draft", "Expired"].map((label, index) => (
							...
							<div className="space-y-3">
								{promoCards.length > 0 ? promoCards.map((promo, index) => (
									<article key={promo.id} className={`grid overflow-hidden rounded-2xl border bg-white shadow-sm md:grid-cols-[160px_minmax(0,1fr)] ${index === 0 ? "border-sky-600 ring-2 ring-sky-100" : "border-slate-200"}`}>
										...
										<div className="flex flex-col gap-2 p-4 text-[10px]">
											...
											<div className="flex gap-1 pt-1">
												<button type="button" onClick={() => setSelectedPromoId(promo.id)} className="rounded-md bg-sky-50 p-1.5 text-sky-600"><Edit3 className="h-3.5 w-3.5" /></button>
												<button type="button" className="rounded-md bg-slate-50 p-1.5 text-slate-500"><Eye className="h-3.5 w-3.5" /></button>
												<button type="button" onClick={() => handleDeletePromo(promo.id)} className="rounded-md bg-rose-50 p-1.5 text-rose-600"><Trash2 className="h-3.5 w-3.5" /></button>
											</div>
										</div>
									</article>
								)) : (
						...
						<section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
							<div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
								...
								<span className="rounded-full bg-emerald-50 px-2 py-1 text-[8px] font-semibold text-emerald-600">{selectedPromo?.url ?? "Kosong"}</span>
							</div>
							<form onSubmit={handleUpdatePromo} className="space-y-3 p-4 text-[10px]">
								<div className="rounded-2xl bg-gradient-to-br from-sky-600 to-sky-800 p-3 text-white">
									<div className="flex items-center gap-3">
										...
										<div><div className="text-[11px] font-medium">{selectedPromo ? `Promo ${selectedPromo.id}` : "Belum ada data promo"}</div><div className="text-[8px] text-white/70">Tampilan di website</div></div>
									</div>
								</div>
								{selectedPromo ? (
									<>
										<button type="button" className="flex w-full flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-slate-500"><Upload className="h-5 w-5" />Upload gambar promo (data URL dari database)</button>
										<div>
											<div className="mb-1 text-[8px] font-semibold uppercase tracking-[0.5px] text-slate-500">URL gambar</div>
											<input value={editForm.url} onChange={(event) => setEditForm({ url: event.target.value })} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 outline-none" />
										</div>
										{actionError ? <div className="rounded-lg bg-rose-50 px-3 py-2 text-[9px] text-rose-600">{actionError}</div> : null}
										<div className="flex gap-2">
											<button type="button" onClick={() => selectedPromo && setEditForm({ url: selectedPromo.url })} className="flex-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-[10px] font-medium text-slate-500">Batal</button>
									...
									<div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-center text-[10px] text-slate-500">Belum ada promo di database.</div>
								)}
							</form>
