...
export default function AdminLayananCrud() {
	...
	const selectedService = useMemo(() => services.find((item) => item.id === selectedServiceId) ?? null, [selectedServiceId, services]);

	useEffect(() => {
		if (selectedService) {
			setEditForm({ nama_layanan: selectedService.nama_layanan, url: selectedService.url });
		}
	}, [selectedService]);
	...
	async function handleCreateService(event: React.FormEvent<HTMLFormElement>) {
		...
		try {
			...
			setSubmitError(error instanceof Error ? error.message : "Gagal menambah layanan");
		} finally {
	...
	async function handleUpdateService(event: React.FormEvent<HTMLFormElement>) {
		...
		try {
			...
			setDeleteError(error instanceof Error ? error.message : "Gagal mengubah layanan");
		} finally {
			setIsSaving(false);
		}
	}
	...
	async function handleDeleteService(serviceId: number) {
		...
		try {
			...
			setDeleteError(error instanceof Error ? error.message : "Gagal menghapus layanan");
		}
	}
	...
	const serviceCards: ServiceItem[] = services.map((item, index) => ({
		...
		iconWrap: index % 2 === 0 ? "bg-sky-50 text-sky-600" : "bg-emerald-50 text-emerald-600",
				...
				<section className="flex min-w-0 flex-col bg-[#F0F4FA]">
					<header className="flex flex-col gap-3 border-b border-slate-200 bg-white px-5 py-3 lg:flex-row lg:items-center lg:justify-between">
						<div>
							...
							<div className="text-[9px] text-slate-500">Data diambil dari tabel layanan pada database</div>
						</div>
						<div className="flex flex-wrap items-center gap-2">
							<button type="button" className="inline-flex items-center gap-1 rounded-full bg-sky-50 px-3 py-1.5 text-[10px] font-medium text-sky-600"><ListFilter className="h-3 w-3" />Semua ({services.length})</button>
					...
					<div className="grid flex-1 gap-4 overflow-y-auto p-4 xl:grid-cols-[minmax(0,1fr)_340px] lg:p-5">
						<section className="space-y-3">
							<form onSubmit={handleCreateService} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm space-y-3">
								<div className="flex items-center justify-between gap-2">
									<div className="text-[12px] font-medium text-slate-900">Tambah layanan</div>
									<span className="rounded-full bg-sky-50 px-2 py-1 text-[8px] font-semibold text-sky-600">POST /api/layanan</span>
								</div>
								<div className="grid gap-2 md:grid-cols-2">
									<div>
										<div className="mb-1 text-[8px] font-semibold uppercase tracking-[0.5px] text-slate-500">Nama layanan</div>
										<input value={form.nama_layanan} onChange={(event) => setForm((current) => ({ ...current, nama_layanan: event.target.value }))} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-[10px] outline-none" placeholder="Contoh: Homevisit" />
									</div>
									<div>
										<div className="mb-1 text-[8px] font-semibold uppercase tracking-[0.5px] text-slate-500">URL / gambar</div>
										<input value={form.url} onChange={(event) => setForm((current) => ({ ...current, url: event.target.value }))} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-[10px] outline-none" placeholder="https://..." />
									</div>
								</div>
							...
							<div className="rounded-2xl border border-dashed border-slate-300 bg-white/70 p-4 text-[9px] text-slate-500">
								{services.length > 0 ? "Drag untuk ubah urutan layanan." : "Belum ada data layanan di database."}
							</div>
							{serviceCards.length > 0 ? serviceCards.map((service) => {
								const Icon = service.icon;

								return (
									<article key={service.id} className={`flex items-center gap-3 rounded-2xl border bg-white p-3 shadow-sm ${service.active ? "border-slate-200" : "border-slate-200 opacity-60"}`}>
										...
										<div className="flex gap-1">
											<button type="button" onClick={() => setSelectedServiceId(service.id)} className="inline-flex h-6 w-6 items-center justify-center rounded-md bg-sky-50 text-sky-600"><Edit3 className="h-3 w-3" /></button>
											<button type="button" onClick={() => handleDeleteService(service.id)} className="inline-flex h-6 w-6 items-center justify-center rounded-md bg-rose-50 text-rose-600"><Trash2 className="h-3 w-3" /></button>
										</div>
									</article>
								);
							}) : (
						...
						<section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
							<div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
								...
								<span className="rounded-full bg-sky-50 px-2 py-1 text-[8px] font-semibold text-sky-600">{selectedService?.nama_layanan ?? "Belum ada data"}</span>
							</div>
							<form onSubmit={handleUpdateService} className="space-y-3 p-4">
								<div className="rounded-2xl bg-sky-50 p-3">
									<div className="flex items-center gap-3">
										...
										<div>
											<div className="text-[11px] font-medium text-slate-900">{selectedService?.nama_layanan ?? "Belum ada data"}</div>
											<div className="text-[9px] text-slate-500">{selectedService?.url ?? "Tambahkan data layanan di database"}</div>
										</div>
									</div>
								</div>
								{selectedService ? (
									<div className="space-y-2">
										<div>
											<div className="mb-1 text-[8px] font-semibold uppercase tracking-[0.5px] text-slate-500">Nama layanan</div>
											<input value={editForm.nama_layanan} onChange={(event) => setEditForm((current) => ({ ...current, nama_layanan: event.target.value }))} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-[10px] outline-none" />
										</div>
										<div>
											<div className="mb-1 text-[8px] font-semibold uppercase tracking-[0.5px] text-slate-500">Deskripsi</div>
											<textarea value={editForm.url} onChange={(event) => setEditForm((current) => ({ ...current, url: event.target.value }))} className="h-16 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-[10px] outline-none" />
										</div>
										<div>
										...
										<div>
											...
											<div className="flex items-center gap-3 rounded-xl bg-gradient-to-br from-sky-600 to-sky-800 p-3 text-white">
												...
												<div className="min-w-0">
													<div className="text-[11px] font-medium">{editForm.nama_layanan}</div>
													<div className="text-[8px] text-white/75">{editForm.url}</div>
												</div>
								...
								{deleteError ? <div className="rounded-lg bg-rose-50 px-3 py-2 text-[9px] text-rose-600">{deleteError}</div> : null}
								<div className="flex gap-2 border-t border-slate-100 px-4 py-3">
									<button type="button" onClick={() => selectedService && setEditForm({ nama_layanan: selectedService.nama_layanan, url: selectedService.url })} className="flex-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-[10px] font-medium text-slate-500">Batal</button>
									<button type="submit" disabled={isSaving} className="flex flex-1 items-center justify-center gap-1 rounded-lg bg-sky-600 px-3 py-2 text-[10px] font-medium text-white disabled:opacity-60"><Save className="h-3.5 w-3.5" />{isSaving ? "Menyimpan..." : "Simpan"}</button>
								</div>
							</form>
						</section>
					</div>
