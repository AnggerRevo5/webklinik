...
export default function GaleriArtikelAdmin() {
  ...
  const galleryItems: GalleryItem[] = gallery.map((item, index) => ({
    ...
    gradientClassName:
      index % 3 === 0
        ? "from-cyan-300 via-sky-400 to-sky-500"
        : index % 3 === 1
        ...
        <section className="flex min-w-0 flex-col bg-[#F0F4FA]">
          ...
          <div className="flex-1 overflow-y-auto p-4 lg:p-5">
            ...
            >
              ...
              {actionError ? (
                <div className="rounded-lg bg-amber-50 px-3 py-2 text-[9px] text-amber-700">
                  {actionError}
                </div>
              ) : null}
            </form>
            ...
            <div className="mb-3 flex flex-wrap items-center gap-2">
              {galleryFilters.map((filter, index) => (
                <button
                  ...
                  className={`rounded-full border px-3 py-1.5 text-[10px] font-medium transition-all duration-300 hover:-translate-y-0.5 ${activeFilter === filter ? "border-sky-600 bg-sky-600 text-white shadow-sm" : "border-slate-200 bg-white text-slate-500 hover:border-sky-200 hover:text-slate-700"}`}
                >
                  {filter}
                </button>
              ))}
