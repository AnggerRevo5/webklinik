...
import { getTiktokStats, type TiktokStatsData } from "@/src/lib/api";
...
/** Widget ringkas statistik TikTok klinik (cache dari RapidAPI, refresh berkala). */
export default function TiktokStats({ className }: { className?: string }) {
  const [stats, setStats] = useState<TiktokStatsData>(null);
  const [available, setAvailable] = useState(true);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let active = true;
    getTiktokStats().then((result) => {
      if (active) {
  ...
  return (
    <div
      className={cn(
        "rounded-2xl bg-linear-to-br from-[#25F4EE] via-[#000000] to-[#FE2C55] p-[1.5px]",
        className,
      )}
    >
      <div className="rounded-2xl bg-white p-4">
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          <div className="rounded-xl bg-[#faf9f7] p-2">
            <div className="flex items-center justify-center gap-1 text-[#3f3f3f]">
